...
public interface CompanyMemberRepository extends JpaRepository<CompanyMember, Long> {
    ...
    List<CompanyMember> findByCompanyOrderByUserEmailAsc(Company company);

    Optional<CompanyMember> findByCompanyAndUser(Company company, User user);

    Optional<CompanyMember> findByIdAndCompany(Long id, Company company);

    boolean existsByCompanyAndUser(Company company, User user);

    long countByCompanyAndRole(Company company, CompanyRole role);
}
